...
int main(int argc, char* argv[])
{
  ...
  try {
	...
	table.doTypeTableAnal();
	table.doGlobalFnAnal();
	Scanner::DepAnal da;
	da.setMapObject(table.fetchBothTables());
	std::cout << "\n=================\n";
	...
	path += "\\";
	da.saveToDBInXml(path+exec.fetchDepXMLFile());
	StrongComponents sc(da);
	std::cout << "\n=================\n";
	std::cout << "Requirement 6:\n";
	std::cout << "=================\n";
	display.displayStrongComponents(sc.fetchStrongComponents());
	NoSQLDB::NoSqlDb<std::string> db;
	NoSQLDB::Persist<std::string> scPersist(db);
	scPersist.saveStrongComp(path+exec.fetchStrongCompXMLFile(), sc.fetchStrongComponents());
  }
