...
Shim::Shim(int rcvPort, Message^ downloadFolder)
{
  ObjectFactory factory;
  pSendr = factory.createSendr();
  pRecvr = factory.createRecvr();
  pMockChan = factory.createMockChannel(pSendr, pRecvr, rcvPort,sysStrToStdStr( downloadFolder));
  pMockChan->start();
}
...
int main()
{
  ...
  Shim^ pShim = gcnew Shim(8080,"downloads");
