...
//ep is the destination endpoint
HttpMessage MsgClient::makeMessage(size_t n, const std::string& body, const EndPoint& ep)
{
  ...
  case 1:
    ...
    msg.addAttribute(HttpMessage::parseAttribute("fromAddr:" + this->endPoint));
 ...
 */
bool MsgClient::sendFile( const std::string& filenamewithwholeMessage, Socket& socket)
{
  // assumes that socket is connected
	std::string filename = searchAttributes(filenamewithwholeMessage, "filePath");
  ...
  msg.addAttribute(HttpMessage::Attribute("catg", searchAttributes(filenamewithwholeMessage,"catg")));
...
//finds the value of attr in msg
std::string MsgClient::searchAttributes(std::string messageBody, std::string attribute)
{
	int attribute1 = (messageBody.find("<" + attribute + ">") + (attribute.length() + 2));
	int attribute2 = messageBody.find("</" + attribute + ">");
	return messageBody.substr(attribute1, attribute2 - attribute1);
}
...
//assign the clientId
void MsgClient::fetchClientId(EndPoint id)
{
	clientId = id;
}

//Assign the endPoint
void MsgClient::fetchEndPoint(EndPoint port)
{
	endPoint = "localhost:" + port;
}

//Send text message from the channel
void MsgClient::sendTextMessage(std::string messageType, std::string messageBody, Socket & socket)
{
	fetchEndPoint(searchAttributes(messageBody, "fromPort"));
	std::string toAddr = "localhost:" + searchAttributes(messageBody, "toPort");
	HttpMessage httpMesssage= makeMessage(1, messageBody, toAddr);
	sendMessage(httpMesssage, socket);	
}

//send file message from the channel
void MsgClient::sendFileMessage(std::string messageBody, Socket & socket)
{
	std::string path = searchAttributes(messageBody, "fPth");
	sendFile(messageBody, socket); //1st parameter should contain the path also
}
