...
*  --------------------
*  ver 5.2 : 20 Apr 17
*  - changed SocketListener::start(CallObj& co) to pass its CallObj
*    to each ClientHandler thread by value, instead of by reference.
*    Now if callable object is not copy-able will get compile-time error.
*    Previous version could have run-time problems due to inadvertant 
*    sharing of callable object state.
*  ver 5.1 : 10 Apr 16
  ...
  std::thread ListenThread(
    [&]()
    {
      ...
      while (!acceptFailed_)
      {
        ...
        // statement below was in ver 5.1:
